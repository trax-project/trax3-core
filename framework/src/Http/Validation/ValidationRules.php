...
class ValidationRules
{
    ...
    /**
     * Time with the format HH:MM or "now".
     *
     * @return void
     */
    protected static function registerTimeOrNowRule()
    {
        app('validator')->extend('time_or_now', function ($attribute, $value, $parameters, $validator) {
            if (!is_string($value)) {
                return false;
            }
            if ($value == 'now') {
    ...
    /**
     * Boolean flex rule (0, 1, false, true, "0", "1", "true", "false" allowed).
     *
     * @return void
     */
    protected static function registerMixedBooleanRule()
    {
        app('validator')->extend('mixed_boolean', function ($attribute, $value, $parameters, $validator) {
    ...
    protected static function registerStrictNumericRule()
    {
        app('validator')->extend('strict_numeric', function ($attribute, $value, $parameters, $validator) {
            return (!is_string($value) && Validator::check($value, 'numeric'));
        }, "Invalid numeric.");
    }
    ...
    protected static function registerStrictIntegerRule()
    {
        app('validator')->extend('strict_integer', function ($attribute, $value, $parameters, $validator) {
            return (!is_string($value) && Validator::check($value, 'integer'));
        }, "Invalid integer.");
    }
